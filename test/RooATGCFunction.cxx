...
void RooATGCFunction::readProfiles(TDirectory& dir) const {
  ...
  for(i=0; i<=6; ++i) {
    ...
    P_dk[i] = new TProfile2D(*(dynamic_cast<TProfile2D *>(dir.Get(TString::Format("p%i_lambda_dk", i)))));
    P_dk[i]->SetName(TString::Format("p%i_lambda_dk", i));
    P_dk[i]->SetDirectory(0);
    if (P_dg1[i]) delete P_dg1[i];
    P_dg1[i] = new TProfile2D(*(dynamic_cast<TProfile2D *>(dir.Get(TString::Format("p%i_lambda_dg1", i)))));
    P_dg1[i]->SetName(TString::Format("p%i_lambda_dg1", i));
