	      ...
	      int    binwidth)
{
  ...
  name            = Form("qqH_%s_Mass_%d",channames[ichan],massgev);
  TH1D   *hVBFout = new TH1D(name,name,nbins,xmin,xmax);
  ...
  int lobin = hggHin->FindFixBin(xmin);
  int hibin = hggHin->FindFixBin(xmax); // one higher than we want
  ...
  for (int ibin=lobin; ibin<hibin; ) {
    ...
    for (int j=0; j<rebin; j++,ibin++) {
      sumggh += binwidth*hggHin->GetBinContent(ibin);
      sumggh += binwidth*hggH2taunuin->GetBinContent(ibin);
      sumvbf += binwidth*hVBFin->GetBinContent(ibin);
    }
    hggHout->SetBinContent(newbin,sumggh);
    hVBFout->SetBinContent(newbin,sumvbf);
  }
  
  allHistFile->WriteTObject(hggHout);
  allHistFile->WriteTObject(hVBFout);
}                                                                      // writesig
