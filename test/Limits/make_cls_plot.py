...
parser.add_option('-b', dest='noX', action='store_true', default=False,
                  help='suppress X windows')
...
print par1, par2, 'input', options.input
...
for file in glob(options.input):
    ...
    # print file, par1, ":", par1val, par2, ":", par2val
    f = TFile.Open(file)
    #ROOT.gDirectory.cd(pwd)
    lTree = f.Get("limit")
    if not lTree:
        f.Close()
        print "didn't find tree limit in file",file
        continue     
    
    for limit in limits:
        (graph, idx) = limits[limit]
        lTree.GetEntry(idx)
...
limconts['+2s'].GetXaxis().SetTitleFont(42)
limconts['+2s'].GetYaxis().SetTitle(par2Latex)
limconts['+2s'].GetYaxis().SetTitleFont(42)
...
def make1DLimit(limits2d, par, parlatex, parmin, parmax, samples, boundScale, isX):
    ...
    limits1d['2s'][0].GetYaxis().SetTitleFont(42)
