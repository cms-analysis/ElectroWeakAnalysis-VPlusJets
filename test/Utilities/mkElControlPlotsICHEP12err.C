...
void mkElControlPlotsICHEP12err(bool domu=false,bool domva=false,
		    bool dovbf=false)
{
    gROOT->ProcessLine(".L tdrstyle.C");
  const double intLUMI = 5100.;
  ...
  const double WZ_scale      = 32.3161   * intLUMI/9996622; // V

  const double QCD_scale     = 1. ; // V
//  const double QCD_scale     = 364000000 *	0.00037 * intLUMI/7529312 ; // V
  const double ZJets_scale   = 3503.71  * intLUMI/962838; // V
  const double ttbar_scale   = 225.197 * intLUMI/6586135; // V
  const double SToppS_scale  = 1.75776  * intLUMI/139974; // V anti-top 
  const double SToppT_scale  = 30.0042  * intLUMI/1935072; // V
  const double SToppTW_scale = 11.1773  * intLUMI/433460; // V
  const double STopS_scale   = 3.89394  * intLUMI/259572; // top
  const double STopT_scale   = 55.531  * intLUMI/23777; //V
  const double STopTW_scale  = 11.1773  * intLUMI/437658; // V
...
//  TCut the_cut("effwt*puwt*((ggdevt==2||ggdevt==3) && fit_status==0) ");
  ...
  TCut the_cut2("effwt*puwt*((ggdevt==2) && fit_status==0  &&abs(JetPFCor_dphiMET[0])>0.8 )");
  TCut the_cut3("effwt*puwt*((ggdevt==3) && fit_status==0  &&abs(JetPFCor_dphiMET[0])>0.8 )");
...
//    H500_file       = new TFile("InData/RD_mu_HWWMH500_CMSSW428.root", "READ");
    ...
    wzShape_file    = new TFile("InData/RD_mu_WZ_CMSSW525.root", "READ");
    if (dovbf)
      wjetsShape_file = new TFile("InData/RD_mu_W4Jets_CMSSW428.root","READ");
    else
      wjetsShape_file = new TFile("InData/RD_mu_WpJ_CMSSW525.root", "READ");
    ttbar_file      = new TFile("InData/RD_mu_TTbar_CMSSW525.root", "READ");
...
//    qcd_file1       = new TFile("InData/RD_mu_QCDMu_CMSSW525.root", "READ");
    ...
    fin2            = new TFile("InData/RD_WenuJets_DataAllSingleElectronTrigger_GoldenJSON_5p1invfb.root", "READ");
...
//    treeh500->Draw(TString(pv.plotvar)+TString(">>th1H500"), the_cut, "goff");
    ...
    TFile* stopps_file =  new TFile("InData/RD_el_STopS_Tbar_CMSSW525.root", "READ");
    TTree* tree64 = (TTree*) stopps_file->Get("WJet");
    TFile* stoppt_file =  new TFile("InData/RD_el_STopT_Tbar_CMSSW525.root", "READ");
    TTree* tree65 = (TTree*) stoppt_file->Get("WJet");
    TFile* stopptW_file =  new TFile("InData/RD_el_STopTW_Tbar_CMSSW525.root", "READ");
