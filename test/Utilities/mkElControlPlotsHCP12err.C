...
void mkElControlPlotsHCP12err(bool domu=false,bool domva=false,
		    bool dovbf=false)
{
    gROOT->ProcessLine(".L tdrstyle.C");
  const double intLUMI = 11900.;
...
//    { "W_electron_pfIsoEA",      0., 0.4, 80, 1,  "W_electron_pfIsoEA",   "W_electron_pfIsoEA",    0.,  .4, 80, 0, 0, 0 },
    { "W_electron_etCorr",     30, 150, 24, 0,  "Electron e_{T} (GeV)", "W_electron_et",   30,  150, 24, 0, 0, 0 }, 
...
//#if 0
    ...
    { "W_mt",             30, 140, 11, 3,   "W Transverse Mass (GeV)", "W_mt",      20,  140, 12, 0, 0, 1 },
    { "W_mtMVA",             30, 140, 11, 3,   "W Transverse Mass (GeV) withMVA MET", "W_mtMVA",      20,  140, 12, 0, 0, 1 },
//    { "abs(event_metMVA_metPhi-W_electron_phi)",  0, 6.28, 20, 3,   " #Delta#phi(lepton,MVA MET) ",  "event_metMVA_lep_dphi",     0, 6.28, 20, 0, 0, 0 },
//    { "abs(event_met_pfmetPhi-W_electron_phi)",  0, 6.28, 20, 3,   " #Delta#phi(lepton,pf MET) ",  "event_pfmet_lep_dphi",     0, 6.28, 20, 0, 0, 0 },
...
//    { "event_met_pfsumet",  0, 2000, 40, 3,   "pf SET (GeV)",  "event_met_pfsumet",     0, 2000, 40, 0, 0, 0 },
    ...
    { "event_met_pfmet",  30, 160, 13, 3,   "pf MET (GeV)",  "event_met_pfmet",     20, 160, 14, 0, 0, 0 },
    { "event_metMVA_met",  30, 160, 13, 3,   "MVA MET (GeV)",  "event_metMVA_met",     20, 160, 14, 0, 0, 0 },
...
//    { "W_electron_et",     30, 150, 24, 0,  "Electron e_{T} (GeV)", "W_electron_et",   30,  150, 24, 0, 0, 0 }, 
//    { "W_electron_etCorr",     30, 150, 24, 0,  "Corr. Electron e_{T} (GeV)", "W_electron_etCorr",   30,  150, 24, 0, 0, 0 }, 
    { "W_electron_eta",  -2.5, 2.5, 10, 1,  "Electron #eta",        "W_electron_eta", -3.,   3., 12, 0, 0, 0 },
    { "W_electron_phi",  -3.14, 3.14, 26, 1,  "Electron #phi",        "W_electron_phi", -3.14, 3.14, 26, 0, 0, 0 },
    { "event_nPV",      0, 50., 50, 1,  "Num PV",            "event_nPV",    0.,  50., 50, 0, 0, 0 },
//    { "W_electron_pfiso_chargedHadronIso/W_electron_et",      0, .2, 50, 1,  "electron_pfiso_chargedHadronIso",   "W_electron_pfiso_chargedHadronIso",    0.,  0.2, 50, 0, 0, 0 },
//    { "W_electron_pfiso_photonIso/W_electron_et",      0, 0.3, 50, 1,  "electron_pfiso_photonIso",            "W_electron_pfiso_photonIso",    0.,  0.3, 50, 0, 0, 0 },
//    { "W_electron_pfiso_neutralHadronIso/W_electron_et",      0, 0.3, 50, 1,  "electron_pfiso_neutralHadronIso",   "W_electron_pfiso_neutralHadronIso",    0.,  0.3, 50, 0, 0, 0 },
//    { "W_electron_mvaTrigV0",      0.9, 1., 50, 1,  "W_electron_mvaTrigV0",   "W_electron_mvaTrigV0",    0.9,  1., 50, 0, 0, 0 },
...
////////////////////////////////////////////VBF
  ...
  //TCut the_cut("effwt*puwt*((evtNJ==2 || evtNJ==3) && (W_mt > 50.) && (JetPFCor_Pt[0]>40.) 
  // && (abs(JetPFCor_Eta[0]-JetPFCor_Eta[1])<1.2) && 
  // (sqrt(JetPFCor_Pt[0]**2+JetPFCor_Pt[1]**2+2*JetPFCor_Pt[0]*JetPFCor_Pt[1]*cos(JetPFCor_Phi[0]-JetPFCor_Phi[1]))>45.) && (JetPFCor_Pt[1]/Mass2j_PFCor>0.3) && 
  //  (JetPFCor_Pt[1]/Mass2j_PFCor<0.7)) && (event_met_pfmet >30)");

 
  //  TCut the_cut("(ggdevt==2&&(Mass2j_PFCor>65&&Mass2j_PFCor<95)&&(fit_mlvjj>170&&fit_mlvjj<200)&&fit_status==0)");
//  TCut the_cut("effwt*puwt*(TopWm>0)");
//  TCut the_cutE("effwt*puwt*puwt*(TopWm>0)");

  TCut the_cut("effwt*puwt*((ggdevt==2||ggdevt==3) && fit_status==0 &&abs(JetPFCor_dphiMET[0])>0.4)");
  TCut the_cutE("effwt*puwt*puwt*((ggdevt==2||ggdevt==3) && fit_status==0 &&abs(JetPFCor_dphiMET[0])>0.4)");

//  TCut the_cut("effwt*puwt*((ggdevt==2) && fit_status==0 &&abs(JetPFCor_dphiMET[0])>0.8&&JetPFCor_dRlepton[0]>0.7&&JetPFCor_dRlepton[1]>0.7)");
//  TCut the_cutE("effwt*puwt*puwt*((ggdevt==2) && fit_status==0 &&abs(JetPFCor_dphiMET[0])>0.8&&JetPFCor_dRlepton[0]>0.7&&JetPFCor_dRlepton[1]>0.7)");

//  TCut the_cut("effwt*puwt*((ggdevt==2) && fit_status==0 &&abs(JetPFCor_dphiMET[0])>0.8&&mva2j350el>0.6)");
//  TCut the_cutE("effwt*puwt*puwt*((ggdevt==2) && fit_status==0 &&abs(JetPFCor_dphiMET[0])>0.8&&mva2j350el>0.6)");

//  TCut the_cut("effwt*puwt*((ggdevt==2||ggdevt==3) && fit_status==0 &&abs(JetPFCor_dphiMET[0])>0.8)");
//  TCut the_cutE("effwt*puwt*puwt*((ggdevt==2||ggdevt==3) && fit_status==0 &&abs(JetPFCor_dphiMET[0])>0.8)");

//  TCut the_cut("effwt*puwt*((ggdevt==2||ggdevt==3) && fit_status==0 &&abs(JetPFCor_dphiMET[0])>0.8&&abs(W_electron_eta)>1.48)");
//  TCut the_cutE("effwt*puwt*puwt*((ggdevt==2||ggdevt==3) && fit_status==0 &&abs(JetPFCor_dphiMET[0])>0.8&&abs(W_electron_eta)>1.48)");
//  TCut the_cut("effwt*puwt*((ggdevt==2||ggdevt==3) && fit_status==0 &&abs(JetPFCor_dphiMET[0])>0.8&&event_runNo<193834)");
//  TCut the_cutE("effwt*puwt*puwt*((ggdevt==2||ggdevt==3) && fit_status==0 &&abs(JetPFCor_dphiMET[0])>0.8&&event_runNo<193834)");
//  TCut the_cutN("effwt*((ggdevt==2||ggdevt==3) && fit_status==0 &&abs(JetPFCor_dphiMET[0])>0.8)");
//  TCut the_cut_data("effwt*puwt*((ggdevt==2||ggdevt==3) && fit_status==0 &&abs(JetPFCor_dphiMET[0])>0.8)");
//  TCut the_cut_data("effwt*puwt*((ggdevt==2||ggdevt==3) && fit_status==0 &&abs(JetPFCor_dphiMET[0])>0.8&&event_runNo<193700)");
//  TCut the_cut_data("effwt*puwt*((ggdevt==2||ggdevt==3) && fit_status==0 &&abs(JetPFCor_dphiMET[0])>0.8)&&event_runNo>=190450&&event_runNo<=193700");
//  TCut the_cut_data("effwt*puwt*((ggdevt==2||ggdevt==3) && fit_status==0 &&abs(JetPFCor_dphiMET[0])>0.8)&&event_runNo>=193701&&event_runNo<=194400");
//  TCut the_cut_data("effwt*puwt*((ggdevt==2||ggdevt==3) && fit_status==0 &&abs(JetPFCor_dphiMET[0])>0.8)&&event_runNo>=194401&&event_runNo<=194800");
//  TCut the_cut_data("effwt*puwt*((ggdevt==2||ggdevt==3) && fit_status==0 &&abs(JetPFCor_dphiMET[0])>0.8)&&event_runNo>=194801&&event_runNo<=195552");
//  TCut the_cut("effwt*puwt*((ggdevt==2||ggdevt==3) && fit_status==0) ");
  if (dovbf)
...
//  TCut the_cut2("effwt*puwt*((ggdevt==2) && fit_status==0  &&abs(JetPFCor_dphiMET[0])>0.8 &&W_electron_pfIsoEA>0.3&&JetPFCor_dRlepton[0]>0.7&&JetPFCor_dRlepton[1]>0.7)");
//  TCut the_cut3("effwt*puwt*((ggdevt==3) && fit_status==0  &&abs(JetPFCor_dphiMET[0])>0.8 &&W_electron_pfIsoEA>0.3&&JetPFCor_dRlepton[0]>0.7&&JetPFCor_dRlepton[1]>0.7)");

//  TCut the_cut2("effwt*puwt*((ggdevt==2) && fit_status==0  )");
//  TCut the_cut3("effwt*puwt*((ggdevt==3) && fit_status==0  )");
...
//    qcd_file1       = new TFile("InData/RDQCD_WenuJets_Isog0p3NoElMVA_1p6invfb.root", "READ");
    qcd_file1       = new TFile("InData/RDQCD_WenuJets_Isog0p3NoElMVA_9p3invfb.root", "READ");
