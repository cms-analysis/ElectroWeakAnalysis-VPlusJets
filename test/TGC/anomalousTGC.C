...
string steps[NVARS] = {"-0.6", "-0.4", "-0.2", "0.0", "0.2", "0.4", "0.6"};



/////////// define all member functions here first ////////////////
void runAnomalousTGC(float lambdaZ);
...
void draw_hist( Int_t varIndex, std::vector<TH1D*> &hist);
...
//   runAnomalousTGC(-0.6);
//   for (float i= -0.4; i<0.41; i += 0.04) runAnomalousTGC(i);
//   runAnomalousTGC(0.6);
...
void runAnomalousTGC(float lambdaZ) {
  ...
  for(int i=0; i<NVARS; ++i) {
    if( (fabs(lambdaZ)< 0.001) || ((fabs(fabs(lambdaZ)-0.2))<0.001) || 
	((fabs(fabs(lambdaZ)-0.4))<0.001) || (fabs((fabs(lambdaZ)-0.6))<0.001) )
      sprintf(temp, "LambdaZ_%.1f_dKgamma_%.1f.root", lambdaZ, 0.2*(i-3));
    else 
      sprintf(temp, "LambdaZ_%.2f_dKgamma_%.1f.root", lambdaZ, 0.2*(i-3));
    fName[i] = string(temp);
...
//   MakeComparisonPlot(label, 8, false);
//   MakeComparisonPlot(label, 9, false);
  fOutput->Close();
  delete fOutput;
}
...
TLegend* legend( std::string label, std::vector<TH1D*> &hist, bool upperleft) {
  ...
  Double_t x1=0.6, y1=0.65;
  if(upperleft) x1=0.22;
  Double_t  x2=x1+0.3, y2=y1+0.27;
  TLegend *l = new TLegend( x1, y1, x2, y2 );
  l->SetHeader(label.c_str());
  char temp[100];

  for(int i=0; i<NVARS; ++i) {
    sprintf(temp, "#Delta#kappa_{#gamma} = %.1f", 0.2*(i-3));
    l->AddEntry(hist[i], temp, "lp");
  }
  l->SetFillColor(0);
  return l;
} // legend
...
void draw_hist( Int_t varIndex, std::vector<TH1D*> &hist){
  ...
  for(int i=0; i<NVARS; ++i) {
    histname = TString(plotname) + TString(steps[i]);
    ...
    // cout << "file name = " << fName[i].c_str() << endl;
    hist[i]->Print();
  }
...
void  MakeComparisonPlot( std::string label, Int_t varIndex, bool doLogY ){
  ...
  std::vector<TH1D*> hist(NVARS+1);
  draw_hist( varIndex, hist);
  TH1D* ratio_hist[NVARS];
  for(int i=0; i<NVARS; ++i) {
    ratio_hist[i] = get_ratio( hist[NVARS], hist[i] );
  }
