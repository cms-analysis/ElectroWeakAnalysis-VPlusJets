...
void fitWWa_el(){
  ...
  TFile* fin     = new TFile("/uscms_data/d2/jdamgov/NTuples_53X_MORIOND13/CMSSW_5_3_2_patch4/src/ElectroWeakAnalysis/VPlusJets/test/Utilities/fracFit/el_a0W_WWA_PhotonEt.root ");
  ...
  TH1D* th1qcd = (TH1D*) fin->Get("th1qcd");
  ...
  th1qcd->GetXaxis()->SetRangeUser(xmin, xmax);
  ...
  RooHistPdf* pdfQCD = makePdf(th1qcd, "pdfQCD");
...
//  double wwaNorm = th1wwa->Integral() + th1wza->Integral();
  ...
  double QCDNorm = th1qcd->Integral();
  ...
  RooArgList* components 
    = new RooArgList(*pdffk, *pdfwwa, *pdfzz, *pdfwajets, *pdfzajets, *pdfTop, *pdfQCD );
  RooArgList* yields = new RooArgList(nfk, nwwa, nzz, nwajets, nzajets, nTop, nQCD);
  ...
  RooGaussian consNQCD("consNQCD","", nQCD, RooConst(QCDNorm),RooConst(0.75*QCDNorm)) ;
  ...
  RooAbsData::ErrorType errorType = RooAbsData::SumW2;
   ...
   totalPdf.plotOn(frame1,ProjWData(*data),Components("pdffk,pdfwwa, pdfzz,pdfzajets,pdfTop,pdfQCD"),
