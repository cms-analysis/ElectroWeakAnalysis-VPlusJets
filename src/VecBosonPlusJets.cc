...
void VecBosonPlusJets::bookProcessIDHistoes(){
 if(verbose) std::cout<<"In VecBosonPlusJets::bookProcessIDHistoes"<<std::endl;
    ...
    ZCandidatesMass[processIDInt]=  new TH1D(histoid, histoid,120,0.,180.);
  ...
  for(unsigned int muIndex=0; muIndex<2; ++muIndex){
	...
	muIso[muIndex][processIDInt] = new TH1D(histoid, histoid,3000,0,10000);
...
if(MET.find(processIDInt)==MET.end()){
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  this->PlotsBeforeSelection(iEvent);
...
void VecBosonPlusJets::PlotsBeforeSelection(const edm::Event& iEvent){
  if(verbose) std::cout<<"In VecBosonPlusJets::PlotsBeforeSelection "<<std::endl;

  ProcessIDBeforeCuts->Fill(processIDIntOriginal, EventWeight); 

  iEvent.getByLabel(DaughterIso1_, DaughterIso[0]);
  iEvent.getByLabel(DaughterIso2_, DaughterIso[1]);
...
void VecBosonPlusJets::fillHistoes(){
	...
	for(std::vector<const reco::Candidate *>::const_iterator it = ZCandidates.begin();it!=ZCandidates.end();++it){
		...
		for(reco::Candidate::const_iterator muonFromZ = (*it)->begin(); muonFromZ< (*it)->end(); ++muonFromZ){
			...
			double dauIsolation = (*DaughterIso[muCount])[muone];
	 		...
	 		muPt[muCount][processIDInt]->Fill(muonFromZ->pt(),EventWeight);
	  		muEta[muCount][processIDInt]->Fill(muonFromZ->eta(),EventWeight);
	  		muPhi[muCount][processIDInt]->Fill(muonFromZ->phi(),EventWeight);
			muIso[muCount][processIDInt]->Fill(dauIsolation,EventWeight);
			++muCount;
     		}
		
    		ZCandidatesMass[processIDInt]->Fill(DiMuMass,EventWeight); 
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  if(METSelection){
	...
	MET[processIDInt]->Fill(caloMET,EventWeight);
	METx[processIDInt]->Fill(caloMETx,EventWeight);
	METy[processIDInt]->Fill(caloMETy,EventWeight);
  ...
  if(JetSelection){
   ...
   for(recoJetCollctnItr=highPtJets.begin();recoJetCollctnItr!=highPtJets.end()
	...
	for(itJet=selectedJets.begin(); itJet!=selectedJets.end();++itJet){
		JetPTAll[processIDInt]->Fill((*itJet)->pt(),EventWeight);
       		JetEtaAll[processIDInt]->Fill((*itJet)->eta(),EventWeight);
       		JetPhiAll[processIDInt]->Fill((*itJet)->phi(),EventWeight);
 		jetCount++;
	 }
